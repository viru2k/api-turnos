...
class MantenimientoController extends Controller
{
  ...
  {
    $this->middleware("auth:api")->except("index", "getSector");
  }
  ...
  {
    $res = DB::select(
      DB::raw("SELECT `id`, `sector_nombre`, `sector_abreviado`, `estado` FROM `sector`
       ")
    );
  ...
  {
    $res = DB::select(
      DB::raw("SELECT `id`, `puesto_nombre` FROM `puesto`
       ")
    );
    return response()->json($res, "200");
  }
  ...
  {
    $res = DB::select(
      DB::raw("SELECT sector.id , `sector_nombre`, `sector_abreviado`, `estado`,sector_usuario.id as sector_usuario_id, sector_usuario.usuario_id, sector_usuario.fecha_ingreso, sector_usuario.puesto_defecto, users.nombreyapellido 
  ...
  public function getSectorUsuarioAsociado(Request $request)
  {
    ...
    $res = DB::select(
     ...
     WHERE  sector_usuario_asociado.sector_id = sector.id  AND users.id = sector_usuario_asociado.usuario_id AND users.id = '" .
          $usuario_id .
          "'
       "
      )
  ...
  {
    DB::table("sector_usuario_asociado")
      ->where("id", "=", $id)
      ->delete();
    return response()->json($id, "200");
  }
  ...
  {
    $res = DB::select(
      DB::raw("SELECT `id`, `regla` FROM `reglas`
   ")
    );
    return response()->json($res, "200");
  }
  ...
  {
    $res = DB::select(
      DB::raw("SELECT reglas.id as regla_id,reglas.regla,  sector_regla.sector_usuario_id, sector_regla.usuario_previo, sector_regla.estado, sector_regla.id as sector_regla_id, sector.sector_nombre, sector_usuario.puesto_defecto  
